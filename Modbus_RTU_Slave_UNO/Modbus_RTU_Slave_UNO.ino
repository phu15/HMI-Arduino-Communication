...
//Crear instancia
Modbus slave(ID, Serial, 23); //ID del nodo. 0 para el master, 1-247 para esclavo
...
void setup() {
  ...
  digitalWrite(13, LOW); //Prende el led del pin 13 (el de la placa)
  Serial.println("Arduino Modbus");
}
...
void io_poll() {
 ...
 // bitWrite( au16data[0], 3, digitalRead( 5 ));
  ...
  au16data[2] = analogRead( 2 ); //The analog value read at pin A0 is saved in au16data [4]. (where 0 = 0v and 1023 = 5v)
